#!/bin/bash
# ============================================================
#  硅基API - Claude Code 一键部署脚本
#  用法: curl -fsSL https://raw.githubusercontent.com/SamAISEO/GuijiAPI/main/install.sh | bash
# ============================================================
...
# 检测是否存在第三方的 api_base_url 或 apiBaseUrl
...
cleanup_third_party() {
  ...
  if [ -f "/etc/claude-code/managed-settings.json" ] && [ -w "/etc/claude-code/managed-settings.json" ]; then
    ...
    warn "检测到 /etc/claude-code/managed-settings.json，但当前无权限删除"
  fi
